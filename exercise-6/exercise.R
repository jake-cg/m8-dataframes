...
MostPopular <- function(my.year){
  names <- name[year == my.year]
  props <- prop[year == my.year]
  popular.name.my.year <- names[props == max(props)]
  return(paste(popular.name.my.year, "is the most popular name in", my.year, sep = " "))
}

# What was the most popular female name in 1994?
popular.name.1994 <- MostPopular(1994)
